...
/** Unified Register File with configurable parameters
  ...
  */
class RegFile(
    dataBits: Int = -1, // Will use XLEN from Config if -1
    regCount: Int = -1, // Will use GPR_NUM from Config if -1
    readPorts: Int = -1, // Will use READ_PORT_NUM from Config if -1
    writePorts: Int = -1 // Will use WRITE_PORT_NUM from Config if -1
) extends Module
    with Config {
  ...
  private val actualReadPorts =
    if (readPorts == -1) READ_PORT_NUM else readPorts
  private val actualWritePorts =
    if (writePorts == -1) WRITE_PORT_NUM else writePorts
