...
class MockMemory(val instructions: Seq[BigInt], val initialData: Map[BigInt, BigInt] = Map()) {
...
class PipelineTest extends AnyFlatSpec with chiseltest.ChiselScalatestTester {
  behavior of "MIPS Pipeline"

  // Helper function to create test instructions
  def makeRTypeInstr(opcode: Int, rs: Int, rt: Int, rd: Int, shamt: Int, funct: Int): BigInt = {
    val op = opcode & 0x3F
    ...
    BigInt((op << 26) | (rs_field << 21) | (rt_field << 16) | (rd_field << 11) | (sh << 6) | fn)
  }
  ...
  it should "correctly handle load-use hazard by stalling" in {
    test(new Core).withAnnotations(Seq(chiseltest.WriteVcdAnnotation)) { dut =>
      // Instructions:
      ...
      val memory = new MockMemory(instructions, Map(4 -> 42))  // Store value 42 at address 4
      ...
      dut.io.ibus_resp_data.poke(instructions(1).U)  // ADD instruction (with hazard)
      ...
      dut.io.ibus_resp_data.poke(instructions(2).U)  // ADD instruction (no hazard)
      dut.clock.step(1)
